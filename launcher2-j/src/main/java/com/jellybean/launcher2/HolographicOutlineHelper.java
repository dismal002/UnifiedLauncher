...
public class HolographicOutlineHelper {
    ...
    static {
        ...
        // Fallback: if Application.onCreate hasn't run yet, use hdpi as safe default
        if (scale <= 0f) {
            scale = 2.0f;
        }
        ...
        sExtraThickOuterBlurMaskFilter = new BlurMaskFilter(Math.max(1f, scale * 12.0f), BlurMaskFilter.Blur.OUTER);
        ...
        sThickInnerBlurMaskFilter = new BlurMaskFilter(Math.max(1f, scale * 4.0f), BlurMaskFilter.Blur.NORMAL);
