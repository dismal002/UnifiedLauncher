...
public class QsbContainerView extends FrameLayout {
    ...
    public static class QsbFragment extends Fragment implements View.OnClickListener {
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            ReceiverCompat.registerNotExported(getActivity(), mRebindReceiver, filter);
        }
        ...
        private View createQsb(LayoutInflater inflater, ViewGroup container) {
            ...
            if (mWidgetInfo == null) {
                ...
                View defaultView = getDefaultView(inflater, container, false);
                launcher.setQsbBar(defaultView);
                SearchDropTargetBar bar = launcher.getSearchDropTargetBar();
                if (bar != null) {
                    bar.setQsbSearchBar(defaultView);
                }
                return defaultView;
            }
            ...
            if (isWidgetBound) {
                ...
                launcher.setQsbBar(mQsb);
                SearchDropTargetBar bar = launcher.getSearchDropTargetBar();
                if (bar != null) {
                    bar.setQsbSearchBar(mQsb);
                }
                return mQsb;
            }

            // Return a default widget with setup icon.
            View defaultView = getDefaultView(inflater, container, true);
            launcher.setQsbBar(defaultView);
            SearchDropTargetBar bar = launcher.getSearchDropTargetBar();
            if (bar != null) {
                bar.setQsbSearchBar(defaultView);
        ...
        public void onActivityResult(int requestCode, int resultCode, Intent data) {
            if (requestCode == REQUEST_BIND_QSB) {
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    ((Launcher) getActivity()).getAppWidgetHost()
