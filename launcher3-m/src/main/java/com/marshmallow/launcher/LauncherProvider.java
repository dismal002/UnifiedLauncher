...
import com.marshmallow.launcher.AutoInstallsLayout.LayoutParserCallback;
...
import com.marshmallow.launcher.config.ProviderConfig;
import com.marshmallow.launcher.util.ManagedProfileHeuristic;
...
public class LauncherProvider extends ContentProvider {
    ...
    protected static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
        ...
        private void addWorkspacesTable(SQLiteDatabase db, boolean optional) {
            ...
            db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_WORKSPACE_SCREENS + " (" +
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            switch (oldVersion) {
                ...
                case 12: {
                    ...
                    mMaxScreenId = 0;
                    addWorkspacesTable(db, true);
                }
                case 13: {
                    // Add appWidgetProvider column (ignore if already exists)
                    try {
                        db.execSQL("ALTER TABLE favorites " +
                                "ADD COLUMN appWidgetProvider TEXT;");
                    } catch (SQLException ex) {
                        Log.w(TAG, "favorites.appWidgetProvider already exists: " + ex.getMessage());
                    }
                }
                case 14: {
                    // Add modified column to favorites (ignore if already exists)
                    try {
                        db.execSQL("ALTER TABLE favorites " +
                                "ADD COLUMN modified INTEGER NOT NULL DEFAULT 0;");
                    } catch (SQLException ex) {
                        // Column already exists — safe to ignore
                        Log.w(TAG, "favorites.modified already exists: " + ex.getMessage());
                    }
                    // Add modified column to workspaceScreens (ignore if already exists)
                    try {
                        db.execSQL("ALTER TABLE workspaceScreens " +
                                "ADD COLUMN modified INTEGER NOT NULL DEFAULT 0;");
                    } catch (SQLException ex) {
                        // Column already exists — safe to ignore
                        Log.w(TAG, "workspaceScreens.modified already exists: " + ex.getMessage());
                    }
                }
                case 15: {
